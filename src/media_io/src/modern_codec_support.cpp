...
ModernCodecPerformanceRequirements ModernCodecDetector::estimate_performance_requirements(
    ...
    req.bandwidth_kbps = static_cast<uint64_t>(static_cast<double>(pixel_count) * 0.1 / static_cast<double>(codec_info.compression_efficiency));
...
bool ModernCodecDetector::validate_streaming_compatibility(const ModernCodecInfo& codec_info,
    ...
    uint64_t estimated_bandwidth = static_cast<uint64_t>(
        static_cast<double>(codec_info.width) * static_cast<double>(codec_info.height) * 0.1 / static_cast<double>(codec_info.compression_efficiency)
    );
